...
const findCanvasComponentsFromDOM = () => {
  try {
    ...
    if (window.__CANVAS_COMPONENTS_EVENTS__) {
      console.log(
        "全局事件存储中的组件ID: ",
        Object.keys(window.__CANVAS_COMPONENTS_EVENTS__)
      );
    }

    // 获取所有画布组件
    const componentElements =
      editorCanvas.querySelectorAll(".canvas-component");
    ...
    const components = Array.from(componentElements).map((el) => {
      ...
      const idMatch = Array.from(el.classList).find((cls) =>
        /component-[\w-]+/.test(cls)
      );
      if (idMatch) {
      ...
      if (!componentType) {
        ...
        else if (el.querySelector(".component-toggle"))
          componentType = "toggle";
        else {
          // 从ID中提取类型前缀
          const typeMatch = componentId.match(
            /^(button|text|input|image|toggle)-/
          );
      ...
      if (window.__CANVAS_COMPONENTS_EVENTS__) {
        ...
        // 尝试不同格式的ID
          ...
          const possibleIds = [
            ...
            componentType
              ? `${componentType}-${originalId.split("-")[1] || ""}`
              : "",
          ].filter(Boolean);
          ...
          if (events.length === 0 && componentType) {
            // 查找以组件类型开头的任何ID
            const matchingId = Object.keys(
              window.__CANVAS_COMPONENTS_EVENTS__
            ).find((id) => id.startsWith(componentType));
...
const updateCanvasComponents = () => {
  ...
  if (props.canvasRef?.value?.canvasComponents) {
    try {
      ...
      // 创建组件列表的深拷贝
      actualCanvasComponents.value = JSON.parse(
...
const openExportModal = () => {
  ...
  try {
    ...
    // 最终确认导出的组件数量和事件数量
    console.log(
      `【最终确认】导出组件数: ${componentsFromDOM.length}, 事件总数: ${totalEventCount}`
    );
    ...
    if (totalEventCount === 0 && window.__CANVAS_COMPONENTS_EVENTS__) {
      ...
      if (globalEventIds.length > 0) {
        ...
        // 尝试将全局存储中的事件数据映射到组件
        if (
          componentsFromDOM.length === globalEventIds.length ||
          globalEventIds.length === 1
        ) {
          console.log("尝试从全局事件存储恢复事件数据...");

          // 如果只有一个组件和一个事件条目，直接使用
          if (componentsFromDOM.length === 1 && globalEventIds.length === 1) {
            componentsFromDOM[0].events = JSON.parse(
              JSON.stringify(
                window.__CANVAS_COMPONENTS_EVENTS__[globalEventIds[0]]
              )
            );
            console.log(
              `已将全局事件 ${globalEventIds[0]} 恢复到组件 ${componentsFromDOM[0].id}`
            );
          }
          // 尝试基于类型匹配
          else {
            for (let i = 0; i < componentsFromDOM.length; i++) {
              ...
              for (const eventId of globalEventIds) {
                if (
                  eventId.includes(comp.type) ||
                  comp.id.includes(eventId) ||
                  (comp.type && eventId.includes(comp.type))
                ) {
                  comp.events = JSON.parse(
                    JSON.stringify(window.__CANVAS_COMPONENTS_EVENTS__[eventId])
                  );
    ...
    if (totalEventCount === 0 && window.__CANVAS_COMPONENTS_EVENTS__) {
      console.log(
        "【全局事件存储内容】",
        JSON.stringify(window.__CANVAS_COMPONENTS_EVENTS__, null, 2)
      );
    }
  ...
  components.forEach((component) => {
    ...
    if (component.events && component.events.length > 0) {
      ...
      component.events.forEach((event) => {
        if (
          !uniqueEventTypes.has(event.type) &&
          event.actions &&
    ...
    switch (component.type) {
      ...
      placeholder="${component.properties.placeholder}"${eventHandlers}
    />\n`;
        break;
    ...
    />\n`;
      ...
      ${
        component.properties.text
          ? `<span class="toggle-label">${component.properties.text}</span>`
          : ""
      }
    </label>\n`;
        break;
...
const generateScriptCode = (components) => {
  ...
  console.log(
    `generateScriptCode: 统计到 ${totalEventCount} 个事件和 ${totalActionCount} 个动作`
  );

  // 生成事件处理函数
  script += "const handleEvent = (componentId, eventType, event) => {\n";
  script +=
    "  console.log(`组件 ${componentId} 的 ${eventType} 事件被触发`, event);\n\n";

  // 检查是否有组件定义了事件
  const hasEvents = components.some(
    (component) => component.events && component.events.length > 0
  );
      ...
      if (component.events && component.events.length > 0) {
        ...
        component.events.forEach((event) => {
          if (!eventTypes.has(event.type)) {
            ...
            // 筛选该事件类型的所有事件
            const eventsOfType = component.events.filter(
              (e) => e.type === event.type
            );
            ...
            console.log(
              `为组件 ${component.id} 的 ${event.type} 事件生成代码, 共 ${totalActionsForType} 个动作`
            );
            ...
            console.log(
              `为组件 ${component.id} 的 ${event.type} 事件生成了 ${generatedActionsCount} 个动作代码`
            );
        ...
        console.log(
          `为组件 ${component.id} 生成了 ${generatedEventTypes} 个事件类型的处理代码`
        );
...
onMounted(() => {
  ...
  document.addEventListener("component-events-updated", (event) => {
    ...
    if (
      actualCanvasComponents.value &&
      actualCanvasComponents.value.length > 0
    ) {
      const componentIndex = actualCanvasComponents.value.findIndex(
        (comp) => comp.id === componentId
      );

      if (componentIndex !== -1) {
        // 更新组件的事件数据
        actualCanvasComponents.value[componentIndex].events = JSON.parse(
